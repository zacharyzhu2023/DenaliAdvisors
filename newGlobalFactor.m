...
% gsecm_test_array = [gsecm_table.gvkey gsecm_table.datadate];
% tokeep = ones(length(gsecm_test_array), 1);
% for ii = 1:length(gsecm_test_array)-1
%     if gsecm_test_array(ii+1, 1) == gsecm_test_array(ii, 1) && gsecm_test_array(ii+1, 2) == gsecm_test_array(ii, 2)
%         tokeep(ii, 1) = 0;
%     end
% end
...
gfunda_table.BE = (gfunda_table.seq + gfunda_table.txditc - gfunda_table.pstk)./gfunda_table.fxrates;
gfunda_table.BE = gfunda_table.BE.*1000000;
...
SG_array_NA = nan(29, 1);
SN_array_NA = nan(29, 1);
SV_array_NA = nan(29, 1);
BG_array_NA = nan(29, 1);
BN_array_NA = nan(29, 1);
BV_array_NA = nan(29, 1);
SG_array_JP = nan(29, 1);
SN_array_JP = nan(29, 1);
SV_array_JP = nan(29, 1);
BG_array_JP = nan(29, 1);
BN_array_JP = nan(29, 1);
BV_array_JP = nan(29, 1);
SG_array_AP = nan(29, 1);
SN_array_AP = nan(29, 1);
SV_array_AP = nan(29, 1);
BG_array_AP = nan(29, 1);
BN_array_AP = nan(29, 1);
BV_array_AP = nan(29, 1);
SG_array_EU = nan(29, 1);
SN_array_EU = nan(29, 1);
SV_array_EU = nan(29, 1);
BG_array_EU = nan(29, 1);
BN_array_EU = nan(29, 1);
BV_array_EU = nan(29, 1);

for jj = 1:12
        temp2 = BEME_cell{12*(ii-1)+1+jj, 2};
        if jj == 6
            bm_stuff = temp2(:, {'gvkey' 'ME' 'BEME', 'region_id'});
        end
...
for ii = 1:29
    ...
    tokeep = ~isnan(bm_stuff.ME) & ~isnan(bm_stuff.BEME) & ~isnan(bm_stuff.region_id);
    bm_stuff = bm_stuff(tokeep, :);
    ret_array = outerjoin(ret_array, bm_stuff, 'keys', {'gvkey'}, 'type', 'left', 'mergekeys', true);
    ret_array(:, 2:4) = [];
    ret_array.Properties.VariableNames{15} = 'ME';
    ret_array.Properties.VariableNames{16} = 'BEME';
    ret_array.Properties.VariableNames{17} = 'region_id';
...
tokeep = ~isnan(BEME_table.BEME);
BEME_table = BEME_table(tokeep, :);
