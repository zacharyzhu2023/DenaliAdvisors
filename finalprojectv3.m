...
for ii = 1:29
    ...
    for jj = 1:2
        ...
        for kk = 1:3
            ...
            temp2 = temp{tokeep, 'ret'};
            if jj == 1 && kk == 1
                SG_array_NA(ii, 1) = nanmean(temp2);
            elseif jj == 1 && kk == 2
                SN_array_NA(ii, 1) = nanmean(temp2);
            elseif jj == 1 && kk == 3
                SV_array_NA(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 1
                BG_array_NA(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 2
                BN_array_NA(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 3
                BV_array_NA(ii, 1) = nanmean(temp2);
            end
        end
    end
    ...
    for jj = 1:2
        ...
        for kk = 1:3
            ...
            temp2 = temp{tokeep, 'ret'};
            if jj == 1 && kk == 1
                SG_array_JP(ii, 1) = nanmean(temp2);
            elseif jj == 1 && kk == 2
                SN_array_JP(ii, 1) = nanmean(temp2);
            elseif jj == 1 && kk == 3
                SV_array_JP(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 1
                BG_array_JP(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 2
                BN_array_JP(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 3
                BV_array_JP(ii, 1) = nanmean(temp2);
            end
        end
    end
    ...
    for jj = 1:2
        ...
        for kk = 1:3
            ...
            temp2 = temp{tokeep, 'ret'};
            if jj == 1 && kk == 1
                SG_array_AP(ii, 1) = nanmean(temp2);
            elseif jj == 1 && kk == 2
                SN_array_AP(ii, 1) = nanmean(temp2);
            elseif jj == 1 && kk == 3
                SV_array_AP(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 1
                BG_array_AP(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 2
                BN_array_AP(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 3
                BV_array_AP(ii, 1) = nanmean(temp2);
            end
        end
    end
    ...
    for jj = 1:2
        ...
        for kk = 1:3
            ...
            temp2 = temp{tokeep, 'ret'};
            if jj == 1 && kk == 1
                SG_array_EU(ii, 1) = nanmean(temp2);
            elseif jj == 1 && kk == 2
                SN_array_EU(ii, 1) = nanmean(temp2);
            elseif jj == 1 && kk == 3
                SV_array_EU(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 1
                BG_array_EU(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 2
                BN_array_EU(ii, 1) = nanmean(temp2);
            elseif jj == 2 && kk == 3
                BV_array_EU(ii, 1) = nanmean(temp2);
            end
        end
    end
...
% HMLs_NA = (prod(SV_array_NA+1)-1) - (prod(SG_array_NA+1)-1);
% HMLs_JP = (prod(SV_array_JP+1)-1) - (prod(SG_array_JP+1)-1);
% HMLs_AP = (prod(SV_array_AP+1)-1) - (prod(SG_array_AP+1)-1);
% HMLs_EU = (prod(SV_array_EU+1)-1) - (prod(SG_array_EU+1)-1);
...
% HMLb_NA = (prod(BV_array_NA+1)-1) - (prod(BG_array_NA+1)-1);
% HMLb_JP = (prod(BV_array_JP+1)-1) - (prod(BG_array_JP+1)-1);
% HMLb_AP = (prod(BV_array_AP+1)-1) - (prod(BG_array_AP+1)-1);
% HMLb_EU = (prod(BV_array_EU+1)-1) - (prod(BG_array_EU+1)-1);

% HML
HML_NA = mean([HMLs_NA, HMLb_NA], 2);
HML_JP = mean([HMLs_JP, HMLb_JP], 2);
HML_AP = mean([HMLs_AP, HMLb_AP], 2);
HML_EU = mean([HMLs_EU, HMLb_EU], 2);
% HML_NA = mean([(prod(HMLs_NA+1)-1) (prod(HMLb_NA+1)-1)]);
% HML_JP = mean([(prod(HMLs_JP+1)-1) (prod(HMLb_JP+1)-1)]);
% HML_AP = mean([(prod(HMLs_AP+1)-1) (prod(HMLb_AP+1)-1)]);
% HML_EU = mean([(prod(HMLs_EU+1)-1) (prod(HMLb_EU+1)-1)]);

% SMB
SMB_NA = mean([SG_array_NA, SN_array_NA, SV_array_NA], 2) - mean([BG_array_NA, BN_array_NA, BV_array_NA], 2);
SMB_JP = mean([SG_array_JP, SN_array_JP, SV_array_JP], 2) - mean([BG_array_JP, BN_array_JP, BV_array_JP], 2);
SMB_AP = mean([SG_array_AP, SN_array_AP, SV_array_AP], 2) - mean([BG_array_AP, BN_array_AP, BV_array_AP], 2);
SMB_EU = mean([SG_array_EU, SN_array_EU, SV_array_EU], 2) - mean([BG_array_EU, BN_array_EU, BV_array_EU], 2);
% SMBbm_NA = mean([(prod(SG_array_NA+1)-1), (prod(SN_array_NA+1)-1), (prod(SV_array_NA+1)-1)]) ...
%                         - mean([(prod(BG_array_NA+1)-1), (prod(BN_array_NA+1)-1), (prod(BV_array_NA+1)-1)]);
% SMBbm_JP = mean([(prod(SG_array_JP+1)-1), (prod(SN_array_JP+1)-1), (prod(SV_array_JP+1)-1)]) ...
%                         - mean([(prod(BG_array_JP+1)-1), (prod(BN_array_JP+1)-1), (prod(BV_array_JP+1)-1)]);
% SMBbm_AP = mean([(prod(SG_array_AP+1)-1), (prod(SN_array_AP+1)-1), (prod(SV_array_AP+1)-1)]) ...
%                         - mean([(prod(BG_array_AP+1)-1), (prod(BN_array_AP+1)-1), (prod(BV_array_AP+1)-1)]);
% SMBbm_EU = mean([(prod(SG_array_EU+1)-1), (prod(SN_array_EU+1)-1), (prod(SV_array_EU+1)-1)]) ...
%                         - mean([(prod(BG_array_EU+1)-1), (prod(BN_array_EU+1)-1), (prod(BV_array_EU+1)-1)]);
...
% check currency conversion - done, same results as john's, maybe check
% more thoroughly
% confirm fx rates - done

% 19870630-20190531, raw
% 19891231-20171231, BEME
% 19900630-21080630, portfolio formation
% 19910630-20190630, returns

% do stuff to adjust for splits - need variable div from john
% think about how to deal with prices that are 0 - prob just remove
